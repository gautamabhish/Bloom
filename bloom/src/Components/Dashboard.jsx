...
const Dashboard = () => {
  ...
  const openFromNotification = (item) => {
    ...
    if (item.userId && !item.poem) {
      ...
      const found = feedData.find((f) => f.id === item.userId || f.username === item.username);
      if (found) {
  ...
  return (
    ...
    {[...Array(150)].map((_, i) => ( <img key={`h-${i}`} src={red_heart} className="absolute w-6 opacity-50" style={{ left: `${Math.random() * 100}%`, animation: `floatUp ${12 + Math.random() * 12}s linear infinite`, }} /> ))} {[...Array(76)].map((_, i) => ( <img key={`b-${i}`} src={physics_balloon} className="absolute w-16 opacity-40" style={{ left: `${Math.random() * 100}%`, animation: `drift ${18 + Math.random() * 18}s linear infinite`, }} /> ))}
      </div>
      ...
      <div className="flex justify-between items-center mb-6 relative z-10 px-2">
        <div>
          ...
          <p className="font-lora text-sm text-[#fafafa]/80">Your spark is live</p>
        </div>
      ...
      <div className="flex flex-col items-center relative z-10">
        {loadingFeed ? (
          ...
          feedData.map((item) => (
            <FeedCard key={item.id || item.username} item={item} onExpand={setExpandedProfile} />
          ))
        )}
      </div>
      ...
      {expandedProfile && (
        <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center px-6 py-12">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setExpandedProfile(null)} />
          <div className="relative z-10 max-w-lg w-full bg-[#fff6e9] rounded-3xl px-8 py-6 overflow-auto" style={{ maxHeight: "85vh" }}>
            <div className="flex justify-between items-start">
              <h2 className="text-2xl font-playfair italic text-[#5b2a2a]">{expandedProfile.username}</h2>
            ...
            <div className="mt-4 font-lora text-sm text-[#4a2c2a] whitespace-pre-wrap leading-relaxed">
              {expandedProfile.poem || expandedProfile.text || "No profile text available."}
            </div>

            <div className="mt-6 flex gap-3">
              <button className="flex-1 py-3 rounded-xl bg-[#f3dede] text-[#8c3037]">Not now</button>
              <button className="flex-1 py-3 rounded-xl bg-[#af323f] text-white">Send spark</button>
            </div>
          </div>
        </div>
      )}
              ...
              {loadingNotif ? (
                ...
                notificationsData.signals.length ? (
                  notificationsData.signals.map((s, idx) => (
                    <button key={idx} className="w-full text-left px-3 py-3 rounded-xl bg-[#fff4f4]" onClick={() => openFromNotification(s)}>
                      <div className="font-playfair italic">{s.username || "Someone nearby"}</div>
                      <div className="text-xs font-lora mt-1 text-[#5b2a2a]/80">{s.hint || "Nearby — check location"}</div>
                    </button>
                  ))
                ) : (
                  <div className="text-sm text-center py-6">No signals nearby right now.</div>
                )
              ) : notifTab === "likes" ? (
                notificationsData.likes.length ? (
                  notificationsData.likes.map((l) => (
                    <button key={l.userId} className="w-full text-left px-3 py-3 rounded-xl bg-[#fff4f4]" onClick={() => openFromNotification(l)}>
                      <div className="font-playfair italic">{l.username}</div>
                      <div className="text-xs font-lora mt-1 text-[#5b2a2a]/80">Sent you a spark</div>
                    </button>
                  ))
                ) : (
                  <div className="text-sm text-center py-6">No pending Sparks.</div>
                )
              ) : (
                notificationsData.resonance.length ? (
                  notificationsData.resonance.map((r) => (
                    <button key={r.userId} className="w-full text-left px-3 py-3 rounded-xl bg-[#fffaf0]" onClick={() => openFromNotification(r)}>
                      <div className="font-playfair italic">{r.username}</div>
                      <div className="text-xs font-lora mt-1 text-[#5b2a2a]/80">It's a Bloom — reveal</div>
                    </button>
                  ))
                ) : (
